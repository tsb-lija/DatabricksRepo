...
# MAGIC
# MAGIC ## The Pipeline
# MAGIC The pipeline we are using in this lesson is located [here]($./Pipeline/ADE 3.1.3 - Generalization).
...
# MAGIC There is a possibility that the pipeline will fail to run with an error related to the storage location. If this happens, please run the following command, substituting your storage location from the output above:  
# MAGIC `dbutils.fs.rm("<YOUR_STORAGE_LOCATION_HERE>", True)`
# MAGIC Then, rerun the `Classroom-Setup` and `generate_pipeline` commands above. This should allow you to successfully run the pipeline.
